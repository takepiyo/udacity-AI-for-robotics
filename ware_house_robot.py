...
def get_goal_coodinate(warehouse, goal_num):
    for i in range(len(warehouse)):
        for j in range(len(warehouse[0])):
            if warehouse[i][j] == goal_num:
                goal = [i, j]
                break
    return goal


def get_cost_map(warehouse, goal):
    cost_map = [
        [1000.0 for i in range(len(warehouse[0]))] for i in range(len(warehouse))
    ]
    is_changed = True
    ...
    while is_changed:
        ...
        for y in range(len(warehouse)):
            for x in range(len(warehouse[0])):
                if y == goal[0] and x == goal[1] and not is_goal_updated:
                    cost_map[y][x] = 0.0
                    is_changed = True
                    is_goal_updated = True
                for is_diagonal, actions in enumerate(move):
                    if is_diagonal:
                        cost = 1.5
                    else:
                        cost = 1.0
                    for action in actions:
                        ...
                        if (
                            x2 >= 0
                            and x2 < len(warehouse[0])
                            and y2 >= 0
                            and y2 < len(warehouse)
                        ):
                            if warehouse[y2][x2] == 0:
                                if cost_map[y2][x2] > cost_map[y][x] + cost:
                                    cost_map[y2][x2] = cost_map[y][x] + cost
                                    is_changed = True
    return cost_map
...
def plan(warehouse, dropzone, todo):
    ...
    for goal_num in todo:
        goal = get_goal_coodinate(warehouse, goal_num)
        cost_map = get_cost_map(warehouse, goal)
        warehouse[goal[0]][goal[1]] = 0
        cost += cost_map[dropzone[0]][dropzone[1]] * 2  # 往復
        # print_list(cost_map)
    return cost
...
true_cost1 = 9
# Test Case 2
warehouse2 = [[1, 2, 3, 4], [0, 0, 0, 0], [5, 6, 7, 0], [0, 0, 0, 8]]
dropzone2 = [3, 0]
todo2 = [2, 5, 1]
true_cost2 = 21
