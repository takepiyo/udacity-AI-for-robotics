...
def estimate_next_pos(measurement, OTHER=None):
    ...
    if len(OTHER) > 1:
        for i in range(len(OTHER) - 1):
            ...
            theta.append(atan2(OTHER[i+1][1] - OTHER[i]
                               [1], OTHER[i+1][0]-OTHER[i][0]))
    else:
        dist = [0]
        theta = [0]
    ...
    xy_estimate = (measurement[0]+avgdist*cos(theta[len(theta)-1]+avgdtheta),
                   measurement[1]+avgdist*sin(theta[len(theta)-1]+avgdtheta))
