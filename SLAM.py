...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    omega_x = matrix(
        [[0.0 for i in range(N + num_landmarks)] for j in range(N + num_landmarks)]
    )
    omega_x.value[0][0] = 1.0
    xi = matrix([[0.0] for i in range(N + num_landmarks)])
    xi.value[0][0] = world_size / 2.0
    omega_y = matrix(
        [[0.0 for i in range(N + num_landmarks)] for j in range(N + num_landmarks)]
    )
    omega_y.value[0][0] = 1.0
    yi = matrix([[0.0] for i in range(N + num_landmarks)])
    yi.value[0][0] = world_size / 2.0
    for time, (observation, position) in enumerate(data):
        omega_x += matrix(
            [
                [1.0 / motion_noise, -1.0 / motion_noise],
                [-1.0 / motion_noise, 1.0 / motion_noise],
            ]
        ).expand(
            N + num_landmarks, N + num_landmarks, [time, time + 1], [time, time + 1]
        )
        xi += matrix(
            [[-position[0] / motion_noise], [position[0] / motion_noise]]
        ).expand(N + num_landmarks, 1, [time, time + 1], [0])
        omega_y += matrix(
            [
                [1.0 / motion_noise, -1.0 / motion_noise],
                [-1.0 / motion_noise, 1.0 / motion_noise],
        ...
        yi += matrix(
            [[-position[1] / motion_noise], [position[1] / motion_noise]]
        ).expand(N + num_landmarks, 1, [time, time + 1], [0])
        for landmark in observation:
            omega_x += matrix(
                [
                    [1.0 / measurement_noise, -1.0 / measurement_noise],
                    [-1.0 / measurement_noise, 1.0 / measurement_noise],
                ]
            ).expand(
                N + num_landmarks,
            ...
            xi += matrix(
                [[-landmark[1] / measurement_noise], [landmark[1] / measurement_noise]]
            ).expand(N + num_landmarks, 1, [time, N + landmark[0]], [0])
            omega_y += matrix(
                [
                    [1.0 / measurement_noise, -1.0 / measurement_noise],
                    [-1.0 / measurement_noise, 1.0 / measurement_noise],
            ...
            yi += matrix(
                [[-landmark[2] / measurement_noise], [landmark[2] / measurement_noise]]
            ).expand(N + num_landmarks, 1, [time, N + landmark[0]], [0])
    mu_x = omega_x.inverse() * xi
    mu_y = omega_y.inverse() * yi
