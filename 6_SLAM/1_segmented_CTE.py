...
class plan:
    ...
    def astar(self):
        ...
        closed = [
            [0 for row in range(len(self.grid[0]))] for col in range(len(self.grid))
        ]
        action = [
            [0 for row in range(len(self.grid[0]))] for col in range(len(self.grid))
        ]
    ...
    def smooth(self, weight_data=0.1, weight_smooth=0.1, tolerance=0.000001):
        ...
        self.spath = [
            [0 for row in range(len(self.path[0]))] for col in range(len(self.path))
        ]
...
class robot:
    ...
    def move(
        self, grid, steering, distance, tolerance=0.001, max_steering_angle=pi / 4.0
    ):
...
class particles:
    ...
    def __init__(
        self, x, y, theta, steering_noise, distance_noise, measurement_noise, N=100
    ):
        self.N = N
    ...
    def get_position(self):
        ...
        for i in range(self.N):
            ...
            orientation += (
                (
                    (self.data[i].orientation - self.data[0].orientation + pi)
                    % (2.0 * pi)
                )
...
def run(grid, goal, spath, params, printflag=True, speed=0.1, timeout=1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        delta = [0.0, 0.0]
        relative_vecter = [0.0, 0.0]
        estimate = filter.get_position()
        for i in range(len(spath[0])):
            delta[i] = spath[index + 1][i] - spath[index][i]
            relative_vecter[i] = estimate[i] - spath[index][i]
        dx2_dy2 = delta[0] ** 2 + delta[1] ** 2
        u = (relative_vecter[0] * delta[0] + relative_vecter[1] * delta[1]) / dx2_dy2
        if u >= 1.0:
            index += 1
        cte = (relative_vecter[1] * delta[0] - relative_vecter[0] * delta[1]) / dx2_dy2
        # ----------------------------------------
